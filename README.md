...
- **RR-Optimized Runner Exits:** entries are an ATR-based zone and SL sits at swing ± ATR, but exits follow the validated RR 1:5 structure instead of a pure win-rate ladder: **`tp_splits[0]` (default 40%) of the position closes at TP1 = 1R**, the stop moves to **breakeven after TP1**, and the remaining **runner is trailed with a Chandelier stop `trail_atr_mult` (default 1.5x) ATR behind the peak** (config: `tp1_r`, `use_trailing_runner`, `trail_atr_mult`) — mirrored exactly in `auto_trades.py` (poll + websocket paths). The 40%/1.5-ATR structure beat the old 20%/3-ATR runner in backtests (better profit factor and expectancy).
...
- **Session Filter:** `session_hours` restricts signals to the best UTC hours (`[]` = off). Multi-window backtesting (60 days / 3 windows) found that restricting to UTC `15–23` keeps **every window positive** (win rate 59–71%) at roughly the same total PnL, by cutting the weakest Asian-morning session (UTC 6–12).
