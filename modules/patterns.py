...
def find_pattern(df):
    ...
    df_idx['min_local'] = df_idx.iloc[argrelextrema(df_idx.low.values, np.less_equal, order=3)[0]]['low']
    df_idx['max_local'] = df_idx.iloc[argrelextrema(df_idx.high.values, np.greater_equal, order=3)[0]]['high']
